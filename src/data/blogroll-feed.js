export const BLOGROLL_FEED_ENTRIES = [
  {
    url: 'http://hintjens.com/blog:16?utm_source=substack&utm_medium=email',
    publishedAt: '2026-04-03',
    thoughts: 'a short interesting read on the importance of simplicity in design, and how it can lead to some powerful unexpected outcomes, ',
  },
  {
    url: 'https://mollyg.substack.com/p/six-rules-for-company-goal-setting',
    publishedAt: '2026-03-04',
    thoughts:
      "this article is interesting because it drives home the importance of ruthless goal setting in driving company success, alignment is always key",
  },
  {
    url: "https://allenpike.com/2026/a-broken-heart",
    publishedAt: "2026-02-16",
    thoughts:
      "this was a really good read.. pretty easy to follow.. aside from the shared downsides and upsides of using coding agents.. also shared some insights in using perfomance inspectors on a mac, which is something i plan on using straight away on a perf problem i have on a project at work..",
  },

  {
    url: "https://www.bitsaboutmoney.com/archive/fraud-investigation/",
    publishedAt: "2026-02-14",
    thoughts:
      "Tbh, most of the concepts and ideas shared in the article went over my head, but it  did highlight to me how stuff are rarely straightforward, black or white, there's always complexity in every domain.",
  },
];
